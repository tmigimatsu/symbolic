...
template <typename ContainerT>
template <bool Const>
CombinationGenerator<ContainerT>::Iterator<Const>::Iterator(
...
template <typename ContainerT>
template <bool Const>
bool CombinationGenerator<ContainerT>::Iterator<Const>::operator==(
  ...
  const bool is_other_end =
      other.it_options_.front() == other.options_->front()->end();
...
CombinationGenerator<ContainerT>::Iterator<Const>::operator*() {
  ...
  if (is_end) {
    throw std::out_of_range(
        "ParameterGenerator::iterator::operator*(): Cannot dereference.");
  }
  return combination_;
}
...
template <bool Const>
typename CombinationGenerator<ContainerT>::template Iterator<Const>
CombinationGenerator<ContainerT>::Iterator<Const>::begin(
    const std::vector<ContainerT*>* options) {
  std::vector<IteratorT> it_options;
...
template <bool Const>
typename CombinationGenerator<ContainerT>::template Iterator<Const>
CombinationGenerator<ContainerT>::Iterator<Const>::end(
    const std::vector<ContainerT*>* options) {
  std::vector<IteratorT> it_options;
  it_options.reserve(options->size());
  for (ContainerT* option : *options) {
...
template <typename ContainerT>
template <typename IteratorT>
CombinationGenerator<ContainerT>::ReverseIterator<IteratorT>::ReverseIterator(
    IteratorT&& it)
    : it_(std::move(it)) {
